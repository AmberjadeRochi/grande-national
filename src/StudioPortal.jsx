  ...
  return (
    <div style={S.app}>
      ...
      <div style={{ background:"#0f0f0f", borderBottom:"1px solid #1e1e1e", padding:"16px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:100 }}>
        <div>
          <div style={{ fontSize:10, letterSpacing:"0.3em", color:"#4ecdc4", textTransform:"uppercase" }}>Studio Portal</div>
      ...
      <div style={{ maxWidth:980, margin:"0 auto", padding:"32px 24px" }}>
        ...
        <div style={{ ...S.card, marginBottom:24, display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:0, padding:0, overflow:"hidden" }}>
          {[
            { label:"Dancers", value:dancers.length, sub:`R${inv.regFees} reg fees`, color:"#4ecdc4" },
            { label:"Solo Entries", value:solos.length, sub:`R${inv.soloFees}`, color:"#e8c547" },
            { label:"Group Entries", value:groups.length, sub:`R${inv.groupFees}`, color:"#a78bfa" },
            { label:"Estimated Total", value:`R${inv.total}`, sub:"excl. final invoice", color:"#ff6b6b" },
          ].map((st,i,arr)=>(
        ...
        <div style={{ display:"flex", gap:4, marginBottom:24, background:"#141414", padding:4, borderRadius:10, border:"1px solid #1e1e1e" }}>
          {[["dancers","👥 Dancers"],["solos","🎭 Solos"],["groups","👯 Groups & Duos"]].map(([key,label])=>(
            <button key={key} onClick={()=>setTab(key)} style={{ flex:1, padding:"10px", borderRadius:8, border:"none", background:tab===key?"#4ecdc4":"transparent", color:tab===key?"#0a0a0a":"#555", cursor:"pointer", fontFamily:"Georgia,serif", fontSize:14, fontWeight:tab===key?"bold":"normal", transition:"all .2s" }}>{label}</button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <>
            ...
            {tab === "dancers" && (
              <div>
                <div style={{ display:"flex", gap:10, marginBottom:16, flexWrap:"wrap" }}>
                  <button style={S.btn("#4ecdc4")} onClick={()=>setShowAddDancer(true)}>+ Add Dancer</button>
                  <button style={S.ghost("#4ecdc4")} onClick={downloadTemplate}>⬇ Download Excel Template</button>
                  <label style={{ ...S.ghost("#4ecdc4"), cursor:"pointer" }}>
                    📥 Import Excel
                        ...
                        <div key={d.id} style={{ ...S.card, padding:"16px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
                          <div style={{ flex:1 }}>
                            ...
                            <div style={{ display:"flex", gap:8, marginTop:8, flexWrap:"wrap" }}>
                              <span style={S.tag("#4ecdc4")}>{dSolos.length} solo{dSolos.length!==1?"s":""}</span>
                              <span style={S.tag("#a78bfa")}>{dGroups.length} group entry/ies</span>
                              <span style={S.tag("#e8c547")}>R{d.registration_fee} reg</span>
                            </div>
                          </div>
            ...
            {tab === "solos" && (
              <div>
                <div style={{ display:"flex", gap:10, marginBottom:16 }}>
                  {dancers.length > 0 && <button style={S.btn("#e8c547")} onClick={()=>setShowAddSolo(dancers[0])}>+ Add Solo Entry</button>}
                </div>
                {solos.length === 0 ? (
                              ...
                              return (
                                <div key={s.id} style={{ ...S.card, padding:"14px 18px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                                  ...
                                  <div style={{display:"flex",gap:8,alignItems:"center"}}>
                                    {url && <button onClick={()=>setPlayingUrl(p=>p===url?null:url)} style={{background:"#e8c54718",border:"none",borderRadius:6,padding:"6px 12px",color:"#e8c547",cursor:"pointer",fontSize:13,display:"flex",alignItems:"center"}}>
            ...
            {tab === "groups" && (
              <div>
                <div style={{ display:"flex", gap:10, marginBottom:16 }}>
                  <button style={S.btn("#a78bfa")} onClick={()=>setShowAddGroup(true)}>+ Add Group / Duo</button>
                </div>
                {groups.length === 0 ? (
                          ...
                          <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12 }}>
                            <div style={{flex:1}}>
                              <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                                ...
                                <span style={S.tag("#e8c547")}>{g.genre}</span>
                              </div>
                              <div style={{fontSize:12,color:"#888",marginTop:4}}>"{g.song_title}" · {g.age_group} · {g.member_count} dancers · R{g.total_fee} total</div>
                              <button onClick={()=>setShowGroupMembers(g)} style={{...S.ghost("#a78bfa"),padding:"4px 12px",fontSize:12,marginTop:10}}>
                                👥 View {members.length} member{members.length!==1?"s":""}
                              </button>
                            </div>
                            <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                              {url && <button onClick={()=>setPlayingUrl(p=>p===url?null:url)} style={{background:"#a78bfa18",border:"none",borderRadius:6,padding:"6px 12px",color:"#a78bfa",cursor:"pointer",fontSize:13,display:"flex",alignItems:"center"}}>
                                {playingUrl===url?"⏸":"▶"}<AudioBars playing={playingUrl===url}/>
                              </button>}
...
function AddDancerModal({ session, onClose, onSave, notify }) {
  ...
  const save = async () => {
    if (!form.first_name||!form.last_name||!form.date_of_birth) { notify("All fields required","#c0392b"); return; }
    setLoading(true);
    try {
  ...
  return (
    <Modal title="Add Dancer" onClose={onClose} color="#4ecdc4">
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        {age !== null && <div style={{padding:"8px 12px",background:"#4ecdc418",borderRadius:8,fontSize:13,color:"#4ecdc4"}}>Age: {age} · {ageGroup}</div>}
        ...
        <button style={S.btn("#4ecdc4")} onClick={save} disabled={loading}>{loading?<Spinner color="#0a0a0a"/>:"Add Dancer"}</button>
      </div>
    </Modal>
  );
}
...
function EditDancerModal({ dancer, onClose, onSave, notify }) {
  ...
  return (
    <Modal title="Edit Dancer" onClose={onClose} color="#4ecdc4">
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        ...
        <button style={S.btn("#4ecdc4")} onClick={save} disabled={loading}>{loading?<Spinner color="#0a0a0a"/>:"Save Changes"}</button>
      </div>
    </Modal>
  );
}
...
function AddSoloModal({ dancer: initialDancer, dancers, session, onClose, onSave, notify }) {
  ...
  const save = async () => {
    if (!dancer||!form.song_title||!form.file) { notify("Select dancer, song title and MP3 file","#c0392b"); return; }
    setLoading(true);
    try {
  ...
  return (
    <Modal title="Add Solo Entry" onClose={onClose} color="#e8c547">
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
...
function EditSoloModal({ solo, onClose, onSave, notify }) {
  ...
  return (
    <Modal title="Edit Solo Entry" onClose={onClose} color="#e8c547">
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        <div style={{padding:"8px 12px",background:"#e8c54718",borderRadius:8,fontSize:13,color:"#e8c547"}}>Dancer: {solo.dancer_name}</div>
        <div><label style={S.label}>Genre</label>
          <select style={S.select} value={form.genre} onChange={e=>set("genre",e.target.value)}>
            {GENRES.map(g=><option key={g}>{g}</option>)}
          </select>
        ...
        <div>
          ...
          <div onClick={()=>document.getElementById("editSoloFile").click()} style={{border:"2px dashed #2e2e2e",borderRadius:10,padding:20,textAlign:"center",cursor:"pointer",background:"#1c1c1c"}}>
            ...
            {form.fileName?<div style={{color:"#e8c547"}}>🎵 {form.fileName}</div>:<div style={{color:"#444",fontSize:13}}>Current: {solo.file_name}<br/><span style={{fontSize:11}}>Click to replace</span></div>}
          </div>
        </div>
        <button style={S.btn("#e8c547")} onClick={save} disabled={loading}>{loading?<Spinner color="#0a0a0a"/>:"Save Changes"}</button>
      </div>
    </Modal>
  );
}
...
function AddGroupModal({ dancers, session, onClose, onSave, notify }) {
  ...
  const save = async () => {
    if (!form.group_name||!form.song_title||selectedMembers.length<2||!form.file) { notify("Fill all fields, select 2+ dancers and upload music","#c0392b"); return; }
    setLoading(true);
    try {
  ...
  return (
    <Modal title="Add Group / Duo Entry" onClose={onClose} color="#a78bfa">
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div>
          ...
          <div style={{maxHeight:200,overflowY:"auto",display:"flex",flexDirection:"column",gap:6}}>
            {dancers.map(d=>{
              ...
              return (
                <div key={d.id} onClick={()=>toggleMember(d)} style={{padding:"10px 14px",borderRadius:8,border:`1px solid ${sel?"#a78bfa":"#2e2e2e"}`,background:sel?"#a78bfa18":"#1c1c1c",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        {memberCount >= 2 && (
          <div style={{padding:"10px 14px",background:"#a78bfa18",borderRadius:8,fontSize:13,color:"#a78bfa"}}>
        ...
        <div>
          ...
          <div onClick={()=>document.getElementById("groupFile").click()} style={{border:"2px dashed #2e2e2e",borderRadius:10,padding:20,textAlign:"center",cursor:"pointer",background:"#1c1c1c"}}>
            ...
            {form.fileName?<div style={{color:"#a78bfa"}}>🎵 {form.fileName}</div>:<div style={{color:"#444",fontSize:13}}>📁 Click to select music file</div>}
          </div>
        </div>
        <button style={S.btn("#a78bfa")} onClick={save} disabled={loading}>{loading?<Spinner color="#fff"/>:"Add Group Entry"}</button>
      </div>
    </Modal>
  );
}
...
function EditGroupModal({ group, dancers, groupMembers, onClose, onSave, notify }) {
  ...
  return (
    <Modal title="Edit Group Entry" onClose={onClose} color="#a78bfa">
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div>
          ...
          <div style={{maxHeight:180,overflowY:"auto",display:"flex",flexDirection:"column",gap:6}}>
            {dancers.map(d=>{
              const sel=!!selectedMembers.find(m=>m.id===d.id);
              return <div key={d.id} onClick={()=>toggleMember(d)} style={{padding:"9px 14px",borderRadius:8,border:`1px solid ${sel?"#a78bfa":"#2e2e2e"}`,background:sel?"#a78bfa18":"#1c1c1c",cursor:"pointer",display:"flex",justifyContent:"space-between"}}>
                <span>{d.first_name} {d.last_name}</span>{sel&&<span style={{color:"#a78bfa"}}>✓</span>}
              </div>;
            })}
          </div>
        </div>
        {memberCount>=2&&<div style={{padding:"8px 12px",background:"#a78bfa18",borderRadius:8,fontSize:13,color:"#a78bfa"}}>{memberCount} dancers · {gType} · R{feePerPerson}pp · Total: R{totalFee}</div>}
        <div>
          ...
          <div onClick={()=>document.getElementById("editGroupFile").click()} style={{border:"2px dashed #2e2e2e",borderRadius:10,padding:16,textAlign:"center",cursor:"pointer",background:"#1c1c1c"}}>
            ...
            {form.fileName?<div style={{color:"#a78bfa"}}>🎵 {form.fileName}</div>:<div style={{color:"#444",fontSize:12}}>Current: {group.file_name} · Click to replace</div>}
          </div>
        </div>
        <button style={S.btn("#a78bfa")} onClick={save} disabled={loading}>{loading?<Spinner color="#fff"/>:"Save Changes"}</button>
      </div>
    </Modal>
  );
}
...
function GroupMembersModal({ group, members, onClose }) {
  return (
    <Modal title={`${group.group_name} — Members`} onClose={onClose} color="#a78bfa">
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        <div style={{padding:"8px 12px",background:"#a78bfa18",borderRadius:8,fontSize:13,color:"#a78bfa",marginBottom:4}}>
        ...
        {members.map((m,i)=>(
          <div key={m.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",background:"#1c1c1c",borderRadius:8,border:"1px solid #242424"}}>
            <div style={{width:28,height:28,borderRadius:"50%",background:"#a78bfa22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:"#a78bfa",fontWeight:"bold"}}>{i+1}</div>
            <div>
